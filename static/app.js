...
document.querySelectorAll('.bd-link').forEach(function (val) {
  val.addEventListener('click', function (e) {
    ...
    fetch("".concat(apiUrl, "/count/").concat(type, "/").concat(e.target.closest('a').dataset.id)).then(function (res) {
      return res.json();
...
document.querySelectorAll('.bd-link-full').forEach(function (val) {
  val.addEventListener('click', function (e) {
...
var adjustAdSize = function adjustAdSize() {
  var width = window.innerWidth;

  if (width >= 375 && width < 768) {
    var ratio = Math.round((width - 30) / 345 * 100) / 100;
    document.getElementById('carousel').style.height = 90 * ratio + 'px';
    document.querySelectorAll('.bd-banner').forEach(function (val) {
      val.style.height = 75 * ratio + 'px';
    });
  }
};

if (mobile_bs.versions.iPhone || mobile_bs.versions.iPad) {
  document.getElementById('addBtn').classList.add('ios');
  document.getElementById('addScreenCloseBtn').classList.add('ios');
  document.getElementById('addBtn').addEventListener('click', function () {
    document.getElementById('addScreen').style.display = 'none';
    addScreenDemo.classList.add('open');
    addScreenMask.classList.add('open');
  });
  ...
  var target = document.querySelectorAll('.ios_link');
  target.forEach(function (val) {
    val.classList.add('link-show');
  });
} else {
  document.getElementById('itemGroup').classList.add('andro');

  var _target = document.querySelectorAll('.andro_link');

  _target.forEach(function (val) {
    val.classList.add('link-show');
  });
}
